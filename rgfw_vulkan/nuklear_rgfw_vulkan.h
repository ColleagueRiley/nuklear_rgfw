 ...
 * ===============================================================
 */
#ifndef NK_RGFW_VULKAN_H_
#define NK_RGFW_VULKAN_H_
...
enum nk_RGFW_init_state { NK_RGFW_DEFAULT = 0, NK_RGFW_INSTALL_CALLBACKS };
...
NK_API void nk_RGFW_font_stash_end(VkQueue graphics_queue);
NK_API void nk_RGFW_new_frame();
NK_API VkSemaphore nk_RGFW_render(VkQueue graphics_queue,
                                   uint32_t buffer_index,
                                   VkSemaphore wait_semaphore,
                                   enum nk_anti_aliasing AA);
NK_API void nk_RGFW_resize(uint32_t framebuffer_width,
                            uint32_t framebuffer_height);
NK_API void nk_RGFW_device_destroy(void);
NK_API void nk_RGFW_device_create(
...
NK_API void                 nk_RGFW_mouse_button_callback(RGFW_window*  window, u8 button, double scroll, b8 pressed);
 ...
 * ===============================================================
 */
#ifdef NK_RGFW_VULKAN_IMPLEMENTATION

#define RGFW_NO_API
#define RGFW_VULKAN
#include <RGFW.h>

#undef NK_RGFW_VULKAN_IMPLEMENTATION
#include <stdlib.h>
...
NK_INTERN uint32_t nk_RGFW_find_memory_index(
...
NK_INTERN void nk_RGFW_create_sampler(struct nk_RGFW_device *dev) {
    VkResult result;
...
NK_INTERN void
nk_RGFW_create_command_pool(struct nk_RGFW_device *dev,
...
NK_INTERN void nk_RGFW_create_command_buffers(struct nk_RGFW_device *dev) {
    VkResult result;
...
NK_INTERN void nk_RGFW_create_semaphore(struct nk_RGFW_device *dev) {
    VkResult result;
...
NK_INTERN void nk_RGFW_create_buffer_and_memory(struct nk_RGFW_device *dev,
                                                 VkBuffer *buffer,
    ...
    alloc_info.memoryTypeIndex = nk_RGFW_find_memory_index(
...
NK_INTERN void nk_RGFW_create_render_pass(struct nk_RGFW_device *dev) {
...
NK_INTERN void nk_RGFW_create_framebuffers(struct nk_RGFW_device *dev,
...
NK_INTERN void nk_RGFW_create_descriptor_pool(struct nk_RGFW_device *dev) {
    ...
    pool_sizes[1].descriptorCount = NK_RGFW_MAX_TEXTURES;

    memset(&pool_info, 0, sizeof(VkDescriptorPoolCreateInfo));
    pool_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_POOL_CREATE_INFO;
    pool_info.poolSizeCount = 2;
    pool_info.pPoolSizes = pool_sizes;
    pool_info.maxSets = 1 + NK_RGFW_MAX_TEXTURES;
...
NK_INTERN void
nk_RGFW_create_uniform_descriptor_set_layout(struct nk_RGFW_device *dev) {
...
NK_INTERN void
nk_RGFW_create_and_update_uniform_descriptor_set(struct nk_RGFW_device *dev) {
...
NK_INTERN void
nk_RGFW_create_texture_descriptor_set_layout(struct nk_RGFW_device *dev) {
    VkDescriptorSetLayoutBinding binding;
    VkDescriptorSetLayoutCreateInfo descriptor_set_info;
    VkResult result;
...
NK_INTERN void
nk_RGFW_create_texture_descriptor_sets(struct nk_RGFW_device *dev) {
    ...
    descriptor_set_layouts = (VkDescriptorSetLayout *)malloc(
        NK_RGFW_MAX_TEXTURES * sizeof(VkDescriptorSetLayout));
    descriptor_sets = (VkDescriptorSet *)malloc(NK_RGFW_MAX_TEXTURES *
                                                sizeof(VkDescriptorSet));

    dev->texture_descriptor_sets =
        (struct nk_vulkan_texture_descriptor_set *)malloc(
            NK_RGFW_MAX_TEXTURES *
    ...
    allocate_info.sType = VK_STRUCTURE_TYPE_DESCRIPTOR_SET_ALLOCATE_INFO;
    allocate_info.descriptorPool = dev->descriptor_pool;
    allocate_info.descriptorSetCount = NK_RGFW_MAX_TEXTURES;
...
NK_INTERN void nk_RGFW_create_pipeline_layout(struct nk_RGFW_device *dev) {
...
nk_RGFW_create_shader(struct nk_RGFW_device *dev, unsigned char *spv_shader,
...
NK_INTERN void nk_RGFW_create_pipeline(struct nk_RGFW_device *dev) {
    ...
    vertex_input_info.stride = sizeof(struct nk_RGFW_vertex);
    ...
    vertex_attribute_description[2].offset =
        NK_OFFSETOF(struct nk_RGFW_vertex, col);
...
NK_INTERN void nk_RGFW_create_render_resources(struct nk_RGFW_device *dev,
                                                uint32_t framebuffer_width,
                                                uint32_t framebuffer_height) {
    nk_RGFW_create_render_pass(dev);
    nk_RGFW_create_framebuffers(dev, framebuffer_width, framebuffer_height);
    nk_RGFW_create_descriptor_pool(dev);
    nk_RGFW_create_uniform_descriptor_set_layout(dev);
    nk_RGFW_create_and_update_uniform_descriptor_set(dev);
    nk_RGFW_create_texture_descriptor_set_layout(dev);
    nk_RGFW_create_texture_descriptor_sets(dev);
    nk_RGFW_create_pipeline_layout(dev);
    nk_RGFW_create_pipeline(dev);
}
...
NK_API void nk_RGFW_device_create(
    ...
    VkDeviceSize max_vertex_buffer, VkDeviceSize max_element_buffer,
    uint32_t framebuffer_width, uint32_t framebuffer_height) {
    struct nk_RGFW_device *dev = &RGFW.vulkan;
    ...
    nk_RGFW_create_sampler(dev);
    nk_RGFW_create_command_pool(dev, graphics_queue_family_index);
    nk_RGFW_create_command_buffers(dev);
    nk_RGFW_create_semaphore(dev);

    nk_RGFW_create_buffer_and_memory(dev, &dev->vertex_buffer,
                                      VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
                                      &dev->vertex_memory, max_vertex_buffer);
    nk_RGFW_create_buffer_and_memory(dev, &dev->index_buffer,
                                      VK_BUFFER_USAGE_INDEX_BUFFER_BIT,
                                      &dev->index_memory, max_element_buffer);
    nk_RGFW_create_buffer_and_memory(
    ...
    nk_RGFW_create_render_resources(dev, framebuffer_width,
                                     framebuffer_height);
}

NK_INTERN void nk_RGFW_device_upload_atlas(VkQueue graphics_queue,
...
NK_INTERN void nk_RGFW_destroy_render_resources(struct nk_RGFW_device *dev) {
    uint32_t i;
...
NK_API void nk_RGFW_resize(uint32_t framebuffer_width,
                            uint32_t framebuffer_height) {
    struct nk_RGFW_device *dev = &RGFW.vulkan;
    RGFWGetWindowSize(RGFW.win, &RGFW.width, &RGFW.height);
    RGFWGetFramebufferSize(RGFW.win, &RGFW.display_width, &RGFW.display_height);
    RGFW.fb_scale.x = (float)RGFW.display_width / (float)RGFW.width;
    RGFW.fb_scale.y = (float)RGFW.display_height / (float)RGFW.height;

    nk_RGFW_destroy_render_resources(dev);
    nk_RGFW_create_render_resources(dev, framebuffer_width,
                                     framebuffer_height);
}
...
NK_API void nk_RGFW_font_stash_end(VkQueue graphics_queue) {
    ...
    nk_RGFW_device_upload_atlas(graphics_queue, image, w, h);
    nk_font_atlas_end(&RGFW.atlas, nk_handle_ptr(dev->font_image_view),
                      &dev->tex_null);
...
NK_API void nk_RGFW_new_frame(void) {
    ...
    if (RGFWGetKey(win, RGFW_KEY_LEFT_CONTROL) == RGFW_PRESS ||
        ...
        nk_input_key(ctx, NK_KEY_COPY,
                     RGFWGetKey(win, RGFW_KEY_C) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_PASTE,
                     RGFWGetKey(win, RGFW_KEY_V) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_CUT,
                     RGFWGetKey(win, RGFW_KEY_X) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_UNDO,
                     RGFWGetKey(win, RGFW_KEY_Z) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_REDO,
                     RGFWGetKey(win, RGFW_KEY_R) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_WORD_LEFT,
                     RGFWGetKey(win, RGFW_KEY_LEFT) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_WORD_RIGHT,
                     RGFWGetKey(win, RGFW_KEY_RIGHT) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_LINE_START,
                     RGFWGetKey(win, RGFW_KEY_B) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_LINE_END,
                     RGFWGetKey(win, RGFW_KEY_E) == RGFW_PRESS);
        nk_input_key(ctx, NK_KEY_TEXT_SELECT_ALL,
                     RGFWGetKey(win, RGFW_KEY_A) == RGFW_PRESS);
    } else {
    ...
    if (ctx->input.mouse.grabbed) {
        RGFWSetCursorPos(RGFW.win, ctx->input.mouse.prev.x,
    ...
    nk_input_button(ctx, NK_BUTTON_LEFT, (int)x, (int)y,
                    RGFWGetMouseButton(win, RGFW_MOUSE_BUTTON_LEFT) ==
                        RGFW_PRESS);
    nk_input_button(ctx, NK_BUTTON_MIDDLE, (int)x, (int)y,
                    RGFWGetMouseButton(win, RGFW_MOUSE_BUTTON_MIDDLE) ==
                        RGFW_PRESS);
    nk_input_button(ctx, NK_BUTTON_RIGHT, (int)x, (int)y,
                    RGFWGetMouseButton(win, RGFW_MOUSE_BUTTON_RIGHT) ==
                        RGFW_PRESS);
    nk_input_button(ctx, NK_BUTTON_DOUBLE, (int)RGFW.double_click_pos.x,
                    (int)RGFW.double_click_pos.y, RGFW.is_double_click_down);
...
NK_API
VkSemaphore nk_RGFW_render(VkQueue graphics_queue, uint32_t buffer_index,
                            VkSemaphore wait_semaphore,
                            enum nk_anti_aliasing AA) {
    struct nk_RGFW_device *dev = &RGFW.vulkan;
    ...
    projection.m[0] /= RGFW.width;
    projection.m[5] /= RGFW.height;
    ...
    render_pass_begin_nfo.renderArea.extent.width = (uint32_t)RGFW.width;
    render_pass_begin_nfo.renderArea.extent.height = (uint32_t)RGFW.height;
    ...
    viewport.width = (float)RGFW.width;
    viewport.height = (float)RGFW.height;
...
NK_API void nk_gflw3_scroll_callback(RGFW_window *win, double xoff,
                                     double yoff) {
    (void)win;
    (void)xoff;
...
NK_API void nk_RGFW_mouse_button_callback(RGFW_window *window, int button,
                                           int action, int mods) {
    double x, y;
    NK_UNUSED(mods);
    if (button != RGFW_MOUSE_BUTTON_LEFT)
        return;
    RGFWGetCursorPos(window, &x, &y);
...
NK_INTERN void nk_RGFW_clipboard_paste(nk_handle usr,
...
NK_INTERN void nk_RGFW_clipboard_copy(nk_handle usr, const char *text,
                                       int len) {
    char *str = 0;
    (void)usr;
    if (!len)
...
nk_RGFW_init(RGFW_window *win, VkDevice logical_device,
              ...
              enum nk_RGFW_init_state init_state,
              VkDeviceSize max_vertex_buffer, VkDeviceSize max_element_buffer) {
    memset(&RGFW, 0, sizeof(struct nk_RGFW));
    RGFW.win = win;
    ...
    nk_RGFW_device_create(logical_device, physical_device,
                           graphics_queue_family_index, image_views,
                           image_views_len, color_format, max_vertex_buffer,
                           max_element_buffer, (uint32_t)RGFW.display_width,
                           (uint32_t)RGFW.display_height);
